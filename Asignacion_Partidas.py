...
def Asignacion_Partidas(usuario,puesto):
  ...
  if procesos_3:
    ...
    perfil=pd.read_sql(f"select perfil from public.usuarios where usuario ='{usuario}'",con)
  ...
  # ----- Bonos y Horas Extras ---- #
    ...
    nombre_3= pd.read_sql(f"select nombre from public.usuarios where usuario ='{usuario}'",con)
    nombre_3 = nombre_3.loc[0,'nombre']
      
    supervisor_3= pd.read_sql(f"select supervisor from public.usuarios where usuario ='{usuario}'",con)
